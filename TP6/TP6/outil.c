...
int ajouter_un_contact_dans_rep(Repertoire* rep, Enregistrement enr)
{
	...
	/////
		...
		if (InsertElementAt(rep->liste, rep->liste->size, enr) != 0) {
			rep->nb_elts++;						// On ajoute l'élement en dernière position, soit en queue !
			modif = true;
			rep->est_trie = false;
			return(OK);
		}
...
void supprimer_un_contact_dans_rep(Repertoire* rep, int indice) {
	...
	if (rep->nb_elts >= 1){				// S'il y a au moins un element dans le tableau, alors...									
		for (int i = indice; i < rep->nb_elts - 1; i++) {   // On décale tout le tableau sur la gauche à partir de "indice" jusque "nb_elts-1".
			rep->tab[i] = rep->tab[i + 1];
		}
		rep->nb_elts--;		// On décrémente d'un élément.
		modif = true;
	}
...
void affichage_enreg(Enregistrement enr)
{
	...
	printf("|");
	printf("%s", enr.prenom);
	for (int i = strlen(enr.prenom); i < MAX_NOM; i++) {       // Afin d'aligner tous les noms et prénoms, on complète avec des espaces jusque max nom...
		printf(" ");
	}
	printf("|");
	printf("%s\n", enr.tel);
...
void affichage_enreg_frmt(Enregistrement enr)
{
	...
	printf("\n|");
	printf("%s", enr.nom);
	for (int i = strlen(enr.nom); i < MAX_NOM; i++) {       // Pour aligner tous les noms et prénoms, on réalise la même chose que précédement!
		printf(" ");
	}
	printf("|");
	printf("%s", enr.prenom);
...
bool est_sup(Enregistrement enr1, Enregistrement enr2)
{
	...
	if (strcmp((tmp3), (tmp4)) < 0) return (true);		 // On fait de même avec les prénoms si jamais les noms sont les mêmes...
...
int rechercher_nom(Repertoire* rep, char nom[], int ind)
{
	...
	while (trouve == false && i < ind_fin) {				  // On va le comparer à chaque nom du répertoire et ceci jusqu'au dernier...

		strncpy_s(tmp_nom2, _countof(tmp_nom2), rep->tab[i].nom, _TRUNCATE);   // On copie dans "tmp_nom2" le nom du répertoire (on le met de même en majuscule!).
...
void compact(char* s)
{
	...
	while (s[i] != '\0') {                  // On parcourt toute la chaine:
		if (s[i] < 0x30 || s[i] > 0x39) {   // Codage ASCII des nombres de 0 à 9, si cela n'y appartient pas , on le supprime!
			j = i;
				...
				if (lire_champ_suivant(buffer, &idx, enr.nom, MAX_NOM, SEPARATEUR) == OK)
				{
					...
					if (lire_champ_suivant(buffer, &idx, enr.prenom, MAX_NOM, SEPARATEUR) == OK)
					{
						...
						if (lire_champ_suivant(buffer, &idx, enr.tel, MAX_TEL, SEPARATEUR) == OK) {
							...
							num_rec++;		// Si l'élément est à priori correct, on le comptabilise.
						}
					}
				}
			/////
